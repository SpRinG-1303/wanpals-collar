        ...
        >
          ...
          {role === null ? (
            ...
            <LoginForm
              ...
              setTab={setTab}
              onBack={() => setRole(null)}
              onSubmit={() => nav({ to: "/onboarding/avatar" })}
              t={t}
            />
          )}
...
function RoleSelect({ onSelect, t }: { onSelect: (r: Role) => void; t: (jp: string, en: string) => string }) {
  return (
    <div className="mt-2">
      <h2 className="text-center font-bold" style={{ fontSize: 22, color: "#2C2C2C" }}>
      ...
      <div className="space-y-4">
        <RoleCard
          ...
          accent="#E8829A"
          accentBg="linear-gradient(135deg, #FFE4EC, #FFF0F4)"
          illustration={<OwnerDoodle />}
          titleJp="いぬの家族"
          titleEn="Pet Family"
          subJp="犬の飼い主の方"
        ...
        <RoleCard
          ...
          accent="#7BB5B0"
          accentBg="linear-gradient(135deg, #E4F2F0, #F0F7F6)"
          illustration={<ClinicDoodle />}
          titleJp="どうぶつ病院"
...
function RoleCard({
  onClick, accent, accentBg, illustration, titleJp, titleEn, subJp, subEn,
}: {
  onClick: () => void; accent: string; accentBg: string;
  ...
  return (
    <button
      ...
      className="w-full rounded-[20px] p-5 flex items-center gap-4 transition-all active:scale-[0.98] text-left"
      style={{
        background: "#FFFFFF",
        border: `2px solid ${hover ? accent : "#F0EAE6"}`,
        boxShadow: hover
    ...
    >
      <div
        ...
        style={{ width: 84, height: 84, background: accentBg }}
      >
        {illustration}
      </div>
      <div className="flex-1 min-w-0">
        <div className="font-bold leading-tight" style={{ fontSize: 20, color: "#2C2C2C" }}>
          {titleJp}
        </div>
        <div className="font-semibold leading-tight mt-0.5" style={{ fontSize: 16, color: accent }}>
          {titleEn}
        </div>
        <div className="mt-1.5" style={{ fontSize: 13, color: "#8A8A8A" }}>
          {subEn} · {subJp}
        </div>
      </div>
    </button>
...
function OwnerDoodle() {
  ...
  return (
    <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Person body */}
      <path d="M10 52c0-8 5-14 12-14s12 6 12 14" fill="#E8829A" stroke="#5C3A2E" strokeWidth="1.4" strokeLinejoin="round" />
      {/* Person eyes */}
      ...
      {/* Cheek blush */}
      <circle cx="17" cy="24" r="1.3" fill="#F2A0B0" opacity="0.7" />
      <circle cx="27" cy="24" r="1.3" fill="#F2A0B0" opacity="0.7" />
      ...
      {/* Collar */}
      <path d="M38 42c4 2 16 2 20 0" stroke="#E8829A" strokeWidth="2" strokeLinecap="round" fill="none" />
    </svg>
  );
}
...
function ClinicDoodle() {
  ...
  return (
    <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* Medical cross badge */}
      <rect x="26" y="42" width="5" height="5" rx="1" fill="#E8829A" />
...
export function PrimaryButton({ children, onClick, accent = "#E8829A" }: { children: React.ReactNode; onClick?: () => void; accent?: string }) {
