...
function Community() {
  ...
  const subs: { jp: string; en: string }[] = [
    { jp: "すべて", en: "All" },
    { jp: "柴犬部", en: "Shiba Club" },
    { jp: "プードル部", en: "Poodle Club" },
    { jp: "迷子情報", en: "Lost Pets" },
    { jp: "獣医Q&A", en: "Vet Q&A" },
    { jp: "東京", en: "Tokyo" },
    { jp: "大阪", en: "Osaka" },
  ];
  ...
  return (
    <AppShell titleJp="👥 コミュニティ" titleEn="👥 Community">
      <div className="flex justify-between items-center -mt-2 mb-3">
        <div className="text-xs text-muted-foreground">{POSTS.length * 412} {t("メンバー", "members")}</div>
        <button className="bg-sakura text-primary rounded-full px-3 py-2 text-xs font-bold flex items-center gap-1 shadow-card">
          <Pencil className="w-3 h-3"/> {t("投稿", "Post")}
        </button>
      </div>

      <div className="flex gap-2 overflow-x-auto scrollbar-hide -mx-4 px-4 pb-2">
        {subs.map((s, i) => (
          <button key={s.en} onClick={() => setSub(i)} className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-bold ${sub === i ? "bg-primary text-primary-foreground" : "bg-card border border-border text-muted-foreground"}`}>
            {t(s.jp, s.en)}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-3 mt-2">
        {POSTS.map((p) => (
          <button key={p.id} onClick={() => setOpen(p.id)} className="w-full text-left bg-card rounded-2xl p-4 shadow-card">
            ...
            <div className="mt-2 font-bold text-sm">{language === "english" ? p.titleEn : p.titleJp}</div>
            {language === "mixed" && <div className="text-xs text-muted-foreground">{p.titleEn}</div>}
      ...
      {post && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end" onClick={() => setOpen(null)}>
          <div className="bg-background w-full max-h-[90vh] rounded-t-3xl p-5 overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 className="font-black text-lg mt-3">{language === "english" ? post.titleEn : post.titleJp}</h2>
            {language === "mixed" && <p className="text-xs text-muted-foreground">{post.titleEn}</p>}
            ...
            <div className="mt-4 bg-sakura-soft border border-sakura rounded-2xl p-3">
              <div className="text-[10px] font-bold text-primary">📡 {t("共有センサーデータ", "Shared sensor data")}</div>
              <div className="flex justify-between text-xs mt-2">
                <div><div className="font-bold">38.8°C</div><div className="text-[10px] text-muted-foreground">{t("体温", "Temp")}</div></div>
                <div><div className="font-bold">{t("2,100歩", "2,100 steps")}</div><div className="text-[10px] text-muted-foreground">{t("運動", "Activity")}</div></div>
                <div><div className="font-bold">82</div><div className="text-[10px] text-muted-foreground">{t("スコア", "Score")}</div></div>
              </div>
            </div>

            <h3 className="mt-4 text-sm font-bold">💬 {t("コメント", "Comments")} ({post.com})</h3>
            <div className="space-y-2 mt-2">
              {[
                ...
                <div key={c.uEn} className="bg-card rounded-xl p-3 shadow-soft">
                  <div className="text-[11px] font-bold">{t(c.uJp, c.uEn)}</div>
                  <div className="text-xs mt-1">{t(c.cJp, c.cEn)}</div>
                </div>
              ))}
            </div>

            <button className="mt-4 w-full bg-primary text-primary-foreground rounded-2xl py-3 font-bold text-sm">🩺 {t("プロに聞く", "Ask a Pro Vet")}</button>
          </div>
        </div>
      )}
    </AppShell>
